...
// (execute_indexed_with_context). Only api.rs old path uses this.
...
pub async unsafe fn execute_indexed_with_context(
    ...
    let config = handle.indexed_config.ok_or_else(|| {
        DataFusionError::Internal("execute_indexed_query_new called without indexed_config".into())
    })?;

    // Classification from Java — no Rust-side inference
    let classification = match (config.tree_shape, config.delegated_predicate_count) {
