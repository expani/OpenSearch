...
///
/// TODO [df55-followup] (OPT-5, https://github.com/apache/arrow-rs/issues/8643):
/// parquet-59 removed the public column-subset decoders `read_columns_indexes`/
/// `read_offset_indexes`, so on a cache miss we now load the WHOLE file's page index
/// (all columns) and keep only the required columns' cells. This is a delta vs v54,
/// which decoded only the predicate/projection columns — so cold-start / first-touch
/// decode is heavier, while warm/steady-state is unchanged (we still cache only the
/// subset). Revisit for true column-subset decode when arrow-rs#8643 (selective
/// metadata decode via reader options) lands.
///
...
mod tests {
    ...
    /// bytes) and delegates all storage to `inner`. Used to prove the OPT-5 (a1)
    /// full-index load still issues ONE whole-region request per index kind — the
    /// fetch-volume property the other `whole_region_fetch_*` tests don't assert.
    #[derive(Debug)]
    ...
    /// OPT-5 (a1) fetch-volume guard: under whole-region fetch, a scoped load must
    /// issue exactly ONE contiguous whole-region `get_ranges` per index kind (CI, OI)
    /// — not many small reads — while keeping the decode scoped to the predicate col.
    #[tokio::test]
